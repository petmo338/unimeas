...
class Chars:
	...
	def write(
		...
		if size not in (1, 2, 3, 4):
			raise ArgumentError('The argument "size" must be 1 or 2 or 3 or 4!')
...
def text_orientation(*text, sep="", orientation, pos, size=1):
	...
	if orientation == "c":
		...
		raise ArgumentError('expected "c", "v" or "r" as orientation, not ', repr(orientation))
	return x
