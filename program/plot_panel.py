...
DATA_LINES = 172800
COLOR_MAP = [
	'#ff6442', '#8781d3', '#fcf05f', '#687f91', '#00cfb6', '#0cc7d3',
...
SI_ACR = {
	'Voltage': 'V', 'Current': 'A', 'Resistance': u"\u2126", 'Time': 's',
	'SampleNumber': '', 'Capacitance': 'F', 'Frequency': 'Hz', 'BIAS': 'V',
	'Temperature': u'\u00B0C', 'Percent': '%', "NOx": "ppm"}
...
class PlotPanel(HasTraits):
	...
	traits_view = View(
		Item(
			'plot_widget', editor=QTGraphWidgetEditor(), show_label=False
		),
		HGroup(
			Label('y-unit: '), 
			Item('selected_y_unit', editor=EnumEditor(name='y_units'), show_label=False),
			Label('x-unit: '), 
			Item('selected_x_unit', editor=EnumEditor(name='x_units'), show_label=False)
		)
	)
	...
	def _plot_widget_default(self):
		...
		self.vLine = pg.InfiniteLine(
			angle=90, movable=False, pen=({'color': '90909080', 'width': 1})
		)
		self.hLine = pg.InfiniteLine(
			angle=0, movable=False, pen=({'color': '90909080', 'width': 1})
		)
		...
		self.label.setPos(
			plot.getPlotItem().getViewBox().viewRect().right(),
	...
	def mouse_moved(self, evt):
		...
		if self.plot_widget.sceneBoundingRect().contains(pos):
			...
			self.label.setText("x=%0.3e,  y=%0.3e" % (mouse_point.x(), mouse_point.y()), color='k')
	...
	def update_visible_plots(self):
		...
		legend_names = [l.text for a, l in self.legend.items]
	...
	def configure_plots(self, instrument):
		...
		self.data = np.zeros(
			shape=(
				len(instrument.output_channels) * (
					len(instrument.x_units) + 
					len(instrument.y_units)), DATA_LINES
			), dtype=np.float32
		)
		for i in range(len(instrument.output_channels)):
			self.plots[instrument.output_channels[i]] = pg.PlotCurveItem(
				x=[0], y=[0],
	...
	def _selected_x_unit_changed(self, unit):
		self.plot_widget.setLabel(
			'bottom', self.x_units[unit], units=SI_ACR.get(self.x_units[unit], unit)
		)
	...
	def _selected_y_unit_changed(self, unit):
		self.plot_widget.setLabel(
			'left', self.y_units[unit], units=SI_ACR.get(self.y_units[unit], unit)
		)
	...
	def add_data(self, data):
		...
		if self.index >= self.plot_size:
			...
			self.data = np.concatenate(
				(self.data, np.zeros(shape=(lines, DATA_LINES), dtype=np.float32)), axis=1
			)
			self.plot_size += DATA_LINES
...
#        logging.getLogger(__name__).info('plots.keys: %s', self.controller.time_plot.plots.keys())
		...
		for channel in self.plots.keys():
			...
			for key in self.y_units:
				# try:
				self.data[
					key + len(channel_data_x) + rows_inc][self.index] \
					= channel_data_y[self.y_units[key]]
				# except KeyError:
			...
			self.plots[channel].setData(
				x=self.data[rows_inc + self.selected_x_unit][:self.index],
