...
PID_configurations = {
    'Normal TO-8': {
        'P': 5.821729083089657, 'I': 1.626933965977756, 'D': 1.332938348953647,
        'N': 1.448274562118016, 'pwm_table_index': 0
    },
    'Sensic M18 ceramic': {
        'P': 9.56699386091245, 'I': 6.07629935945335, 'D': 1.98583413086793,
        'N': 37.8230336021021, 'pwm_table_index': 1
    },
    'L-A Cap': {
        'P': 8.56699386091245, 'I': 6.07629935945335, 'D': 1.98583413086793,
        'N': 37.8230336021021, 'pwm_table_index': 0
    },
    'Small mass': {
        'P': 2.56699386091245, 'I': 6.07629935945335, 'D': 1.98583413086793,
        'N': 37.8230336021021, 'pwm_table_index': 0
    }
}
BAUD_RATE = 115200
...
def temp_to_resistance(temp, resistance_at_rt=109.7):
    return (
        100.0 * (resistance_at_rt / 109.7)) * (1 + (3.908e-3 * temp) + (-5.775e-7 * temp * temp))
...
class TableEntry(HasTraits):
    ...
    def _time_changed(self, new):
        ...
        self.remaining = new
        self.trait_set(
            trait_change_notify=False, ramp=int((self.end_temp - self.start_temp) / (new / 60.0)))

    def _ramp_changed(self, new):
        if new != 0:
            self.trait_set(
                trait_change_notify=False, time=int(60.0 * (self.end_temp - self.start_temp) / new))
            self.remaining = self.time

    @on_trait_change('start_temp', 'end_temp')
    def _update_ramp(self, obj, old, new):
        self.trait_set(
            trait_change_notify=False, ramp=int(
                (self.end_temp - self.start_temp) / (self.time / 60.0)
            )
        )
...
class TemperatureControlPanel(HasTraits):
    ...
    traits_view = View(
        ...
        Group(Item('table_entries', show_label=False, editor=table_editor,
                   enabled_when='True'), HGroup(
            Item('filename'), spring, Item('save', show_label=False),
    ...
    def _save_detailed_log_changed(self, new):
        if new:
            ...
            self.csv_writer.writerow([
                'pv', 'pid_out', 'pwm', 'max31ctrl', 'max31err', 'sp', 'sv', 'RT_ref', 'pid_p',
    ...
    def _on_timer(self):
        ...
        if self.current_row >= len(self.table_entries):
            return
        time_left = \
            self.table_entries[self.current_row].time + self.row_start_time - self.current_time
    ...
    def calculate_temperature_table(self):
        ...
        for row in self.table_entries:
            self.temperature_table = np.hstack(
                (self.temperature_table, np.linspace(row.start_temp, row.end_temp, row.time))
            )
    ...
    def _set_parameters_fired(self):
        self.set_parameters_queue.put_nowait(
            struct.pack(
                '<c5B5f', 's', 0, 0, 0, 0, self.pwm_table_index,
    ...
    def _update_pid_fired(self):
        self.set_parameters_queue.put_nowait(
            struct.pack('<c4f', 'p', self.pid_P, self.pid_I, self.pid_D, self.pid_N))
        self.set_parameters_queue.put_nowait(
            struct.pack(
                '<c5B5f', 's', 0, int(self.use_direct_pwm), 0,
    ...
    def _connect_fired(self):
        if self.serial_handler is None:
            self.serial_handler = SerialHandler(
                self.set_parameters_queue, self.get_parameters_queue,
    ...
    def _set_temp_table_for_calibration(self):
        ...
        self.table_entries = [
            TableEntry(time=60, start_temp=t, end_temp=t, remaining=-1) for t in pwms
            ]
